...
The incremental read spanning the update commit should return the updated
TXN-001 row.

# Why the bounds are the *next* commit's timestamp

On timeline layout v2 — which every v9 table uses — a window bounds a commit's
COMPLETION time (Hudi 1.x parity), while `commit.timestamp` is its REQUESTED
time. The two interleave: the insert is requested at …657661 and completes at
…659468, the update is requested at …700696 and completes at …701669. So the
window `(commits[1].timestamp, commits[2].timestamp]` = `(…700696, …702545]`
admits exactly the update, whose completion sits between them — where
`(commits[0], commits[1]]` admits the insert instead.
"""
...
def _window_around_the_update(table: HudiTable) -> tuple[str, str]:
    """Bounds admitting exactly the update commit. See the module docstring."""
    commits = _commits(table)
    assert len(commits) >= 3
    return commits[1].timestamp, commits[2].timestamp


def test_table_incremental_read_returns_correct_data():
    table = HudiTable(get_test_table_path("v9_txns_simple_nometa", "cow"))
    start, end = _window_around_the_update(table)
...
def test_compute_table_stats_returns_none_for_incremental():
    table = HudiTable(get_test_table_path("v9_txns_simple_nometa", "cow"))
    commits = _commits(table)
    ...
    assert table.compute_table_stats(options) is None
