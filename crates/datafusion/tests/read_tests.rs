...
async fn verify_plan(
    ...
    // The struct field filter remains in FilterExec, simpler predicates go to DataSourceExec.
    assert!(
    ...
    // Verify predicate pushdown is working - some predicates pushed to DataSourceExec
    assert!(
    ...
    assert!(
        ...
        "Name filter should be present in plan (either FilterExec or predicate pushdown)"
    );
