...
impl TableBuilder {
    ...
    pub async fn build(&mut self) -> Result<Table> {
        ...
        // describe HOW to read (which merge engine, read-optimized, batch size)
        // and are exactly the kind of thing an operator sets once, in
        ...
        // `hoodie.read.merge.engine` never reached the reader, so the strict
        // engine parse it exists to protect could not fire and a typo read with
        // the other engine after all.
...
mod tests {
    ...
    /// `hoodie.read.merge.engine` set on the table, in `hoodie.properties`, or in
    /// `hudi-defaults.conf` never reached the file group reader — and because it
    /// never reached the reader, the strict parse guarding against a typoed
    /// engine name could not fire either.
    #[test]
    fn test_keep_at_table_level_keeps_how_to_read_and_drops_which_read() {
        for config in [
            HudiReadConfig::MergeEngine,
