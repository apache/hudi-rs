...
mod tests {
    ...
    #[test]
    fn hfile_format_is_accepted_outside_a_metadata_table() {
        ...
        // Deliberately not a metadata-table path: this used to be the sole reason
        // the format was refused.
        options.insert("hoodie.base.path".to_string(), "/data/my_table".to_string());

        validate_configs(&HudiConfigs::new(options)).expect(
            "an HFile base file is a reader capability, not a property of where the table sits",
        );
    }
