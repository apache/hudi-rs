...
impl Storage {
    ...
    /// Read a Parquet file with projection options (column pruning at I/O level).
    ///
    /// Uses the same projection logic as `get_parquet_file_stream()` via `ProjectionMask`,
    /// but collects all batches into a single `RecordBatch`.
    pub async fn get_parquet_file_data_projected(
        ...
        if let Some(ref column_names) = options.projection {
            ...
            let all_cols: Vec<&str> = arrow_schema
                ...
                .collect();
            log::debug!(
                "Parquet projected read for '{relative_path}': \
                 file schema [{file_schema}] ({n_file} cols), \
                 projecting [{proj}] ({n_proj} cols)",
                file_schema = all_cols.join(", "),
                n_file = all_cols.len(),
        ...
        if batches.is_empty() {
            // Build schema from column_names if projection was set
            if let Some(ref column_names) = options.projection {
                let full_schema = self.get_parquet_file_schema(relative_path).await?;
                let fields: Vec<_> = column_names
                    .iter()
                    .filter_map(|n| full_schema.field_with_name(n).ok().cloned())
                    .collect();
                return Ok(RecordBatch::new_empty(Arc::new(
                    arrow_schema::Schema::new(fields),
                )));
            }
            let schema = self.get_parquet_file_schema(relative_path).await?;
