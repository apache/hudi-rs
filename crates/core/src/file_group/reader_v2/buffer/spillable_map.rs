...
// Only the RocksDB tier serializes; without it there is nothing to encode.
...
/// evicts whole source batches. For a batch whose surviving rows are a
...
/// (matching Hudi's own spill configuration).
#[derive(Debug, Clone)]
...
/// Per-source-batch pin accounting.
///
...
/// summed true retained bytes. See [`PinnedBatch`].
///
...
pub struct SpillableRecordMap {
    ...
    /// true retained bytes. The TRUE-retained budget trigger uses this.
...
impl SpillableRecordMap {
    ...
    /// The TRUE retained heap of the in-memory tier: the sum of the
    ...
    /// heap. New entries that push over this must evict whole source
    ...
    /// Track the peak TRUE-retained in-memory bytes.
    ...
    ///    [`EVICTION_LOW_WATER_FRACTION`] of the budget;
...
mod rocksdb_disk_tier {
    ...
    impl RocksDbDiskMap {
        ...
        /// `staging` and are flushed in `SPILL_BATCH_ROWS`-row batches; deletes go
