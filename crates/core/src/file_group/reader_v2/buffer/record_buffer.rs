...
///   counterpart of Java's `baseFileIterator`. ENG-42992 (A3) replaced the
...
///   base records become zero-copy `BatchRef`s into the streamed batch (A2).
...
pub struct FileGroupRecordBuffer {
    ...
    /// (A1, ENG-42993). Replaces the previously unbounded in-memory `HashMap`.
    ...
    // ── Base file iteration state (ENG-42992 lazy source + A2 batch-ref) ──
    ...
    ///
    /// A3 (ENG-42992) — the base file is pulled one parquet row-group at a
    ...
    /// views (A2) into the *streamed* batch rather than per-row clones during
...
impl FileGroupRecordBuffer {
    ...
    ///
    /// A3 (ENG-42992) — was previously an indexed walk over an eager
    ...
    /// build a zero-copy
    /// [`RecordPayload::BatchRef`](crate::file_group::reader_v2::buffered_record::RecordPayload::BatchRef)
    /// (A2) into the *streamed* batch instead of slicing/cloning a single-row
...
mod tests {
    ...
    // ── A3 (ENG-42992): streamed base source + A2 batch-ref interaction ──────
