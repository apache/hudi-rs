...
#[cfg(test)]
mod position_column_tests {
    use super::*;
    use arrow_array::{ArrayRef, Int32Array, StringArray};
    use arrow_schema::{DataType, Field, Schema};
    use std::sync::Arc;

    /// Position merge needs the parquet virtual row-number column on the base
    /// read. Its absence is an error naming the column, not a silent fall back
    /// to key merge — the buffer has already keyed its records by position, so
    /// carrying on would match them against nothing.
    #[test]
    fn test_base_row_position_array_missing_column_is_an_error() {
        let batch = RecordBatch::try_new(
            Arc::new(Schema::new(vec![Field::new("k", DataType::Utf8, false)])),
            vec![Arc::new(StringArray::from(vec!["a"])) as ArrayRef],
        )
        .unwrap();
        let err = base_row_position_array(&batch)
            .expect_err("a base batch without the row-index column must be an error");
        assert!(
            err.to_string().contains(ROW_INDEX_TEMPORARY_COLUMN_NAME),
            "the error must name the missing column, got: {err}"
        );
    }

    /// A row-index column of the wrong width is refused rather than reinterpreted
    /// — positions are file offsets and an i32 would silently truncate a large
    /// base file.
    #[test]
    fn test_base_row_position_array_wrong_type_is_an_error() {
        let batch = RecordBatch::try_new(
        ...
        .unwrap();
        let err = base_row_position_array(&batch)
            .expect_err("a non-Int64 row-index column must be an error");
        assert!(
            err.to_string().contains("expected Int64"),
