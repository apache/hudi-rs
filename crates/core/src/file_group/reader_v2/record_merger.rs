...
/// is NOT default (GAP-2): `Integer(0).equals(Long(0))` is `false` in Java, so a
...
impl BufferedRecordMerger for EventTimeRecordMerger {
    ...
    ///
    /// The newer (log) record wins unless it is NOT a commit-time-ordering delete
    ...
    fn final_merge(
        ...
        if is_commit_time_ordering_delete(newer_record) {
            return Ok(newer_record.clone());
        }
        let choose_older = !is_commit_time_ordering_delete(older_record)
            && match (&older_record.ordering_value, &newer_record.ordering_value) {
                (Some(old_val), Some(new_val)) => {
                    is_same_class(old_val, new_val) && old_val > new_val
                }
                _ => false,
            };
        if choose_older {
...
mod tests {
    ...
    /// A3c (ENG-38318): EVENT_TIME ordering by a `double` field (e.g. `weight`).
