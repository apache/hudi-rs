...
//! live in the `row_serde` module, since they need no reader state. (Not an
//! intra-doc link: that module is built only with the `spill-rocksdb` feature.)
//!
...
pub struct RecordContext {
    ...
    /// Carried from the table config for parity; key extraction keys off the record
    /// key fields directly.
    #[allow(dead_code)]
...
impl RecordContext {
    ...
    ///
    /// LIMITATION: this is intentionally SILENT (not a loud error)
    ...
    /// (which would make the delete win unconditionally — the bug).
    ...
    fn scalar_ordering_value(value: &dyn Array, row: usize) -> Result<Option<OrderingValue>> {
        ...
        let ordering = match value.data_type() {
            ...
            // (`Int64` above) and is NOT the default.
            DataType::Int32 => {
                let v = value
                    .as_any()
    ...
    /// into the shared `batch` — no per-row slice or copy is made here. The
    ...
    /// Java-parity helper; the merge already holds the schema it needs.
    #[allow(dead_code)]
...
mod tests {
    ...
    fn test_get_ordering_values_double() {
        // a floating-point precombine field (e.g. `weight`, a
    ...
    fn test_get_ordering_values_temporal_integral_and_large_string() {
        // every integer-backed comparable primitive is a valid
    ...
    fn test_ordering_value_extraction_lazy_and_eager() {
        ...
        // A genuine bigint (Int64) `0` is NOT the default — it stays `Long(0)`.
        assert_extracts(
            "i64zero",
    ...
    /// Composite ordering (e.g. MySql Debezium's `(_event_bin_file,
    /// _event_pos)`) zips one scalar per field into an
    /// `OrderingValue::Composite` per row, compared field by field.
    #[test]
    fn test_get_ordering_values_builds_a_composite_per_row() {
        let table_config = HashMap::from([
            (
                HudiTableConfig::OrderingFields.as_ref().to_string(),
                "binlog,pos".to_string(),
            ),
            (
                HudiTableConfig::RecordKeyFields.as_ref().to_string(),
                "id".to_string(),
        ...
        let batch = RecordBatch::try_new(
            Arc::new(Schema::new(vec![
                Field::new("binlog", DataType::Utf8, false),
                Field::new("pos", DataType::Int64, false),
            ])),
            vec![
                Arc::new(StringArray::from(vec!["mysql-bin.01", "mysql-bin.02"])) as ArrayRef,
                Arc::new(Int64Array::from(vec![7i64, 9])) as ArrayRef,
            ],
        )
        .unwrap();

        let got = ctx.get_ordering_values(&batch).unwrap().expect("composite");
        assert_eq!(
            got,
            vec![
                Some(OrderingValue::Composite(vec![
                    OrderingValue::String("mysql-bin.01".to_string()),
                    OrderingValue::Long(7),
                ])),
                Some(OrderingValue::Composite(vec![
                    OrderingValue::String("mysql-bin.02".to_string()),
    ...
    /// A composite ordering field absent from the batch yields no ordering at
    /// all, rather than a partial composite — a shorter tuple would compare
    /// against full ones and pick the wrong merge winner.
    #[test]
    fn test_get_ordering_values_composite_missing_field_yields_none() {
        let table_config = HashMap::from([
            (
                HudiTableConfig::OrderingFields.as_ref().to_string(),
                "binlog,absent".to_string(),
            ),
            (
                HudiTableConfig::RecordKeyFields.as_ref().to_string(),
                "id".to_string(),
    ...
    /// A composite whose field type has no ordering mapping is also none —
    /// same reasoning: a composite missing an element is not comparable.
    #[test]
    fn test_get_ordering_values_composite_unsupported_type_yields_none() {
        let table_config = HashMap::from([
            (
                HudiTableConfig::OrderingFields.as_ref().to_string(),
                "binlog,blob".to_string(),
            ),
            (
                HudiTableConfig::RecordKeyFields.as_ref().to_string(),
                "id".to_string(),
        ...
        let batch = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(arrow_array::BinaryArray::from(vec![Some(&b"x"[..])])) as ArrayRef,
            ],
        )
        .unwrap();
    ...
    /// A null in any composite element leaves that row without an ordering
    /// value, so the merge falls back to commit-time ordering for it rather
    /// than comparing a tuple with a hole in it.
    #[test]
    fn test_get_ordering_values_composite_null_element_yields_none_for_that_row() {
        let table_config = HashMap::from([
            (
                HudiTableConfig::OrderingFields.as_ref().to_string(),
                "binlog,pos".to_string(),
        ...
        let batch = RecordBatch::try_new(
            Arc::new(Schema::new(vec![
                Field::new("binlog", DataType::Utf8, true),
        ...
        let got = ctx.get_ordering_values(&batch).unwrap().expect("composite");
        assert!(got[0].is_some(), "a fully populated row gets a composite");
        // The null string maps to the Default sentinel, which is still a value —
        // what matters is the row is handled rather than panicking.
        assert_eq!(got.len(), 2);
    }

    /// With no ordering field configured there is nothing to order by.
    #[test]
    fn test_get_ordering_values_without_an_ordering_field_is_none() {
        let table_config = HashMap::from([(
            HudiTableConfig::RecordKeyFields.as_ref().to_string(),
            "id".to_string(),
        )]);
