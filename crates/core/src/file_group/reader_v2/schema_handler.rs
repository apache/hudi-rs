...
impl FileGroupReaderSchemaHandler {
    ...
    pub fn prepare_required_schema(
        ...
        // Avro JSONs are available (FFI path), compute required_schema_json by
        ...
        self.required_schema_json = None;
        if let (Some(data_json), Some(requested_json), Some(required_arrow)) = (
            ...
            // recoverable condition. Falling back would leave required_schema_json
            // = None, which is the documented "legacy non-FFI caller" sentinel
            ...
            self.required_schema_json = Some(required_json);
        }
...
mod tests {
    ...
    fn test_prepare_required_schema_avro_json_matches_arrow_land_for_nested_schema() {
        ...
        avro_land
            ...
            .unwrap();
        assert!(avro_land.required_schema_json.is_some());
    ...
    /// contain each name exactly once. Mirrors gold's appendFields semantics
