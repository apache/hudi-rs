...
/// The protocol fields `build_block` encodes by number, named so the layout can
/// be read without cross-referencing the Java format spec. Each mirrors an enum
/// discriminant in [`log_block`](crate::file_group::log_file::log_block).
mod wire {
    /// `LogBlockVersion::V3`, the version modern Hudi tables write.
    pub const BLOCK_VERSION_V3: u32 = 3;
    /// `LogFormatVersion` — the log format this block claims.
    pub const LOG_FORMAT_VERSION: u32 = 1;
    /// `BlockType::AvroData`.
    pub const BLOCK_TYPE_AVRO_DATA: u32 = 3;
    /// `BlockMetadataKey::InstantTime`.
    pub const HEADER_KEY_INSTANT_TIME: u32 = 0;
    /// `BlockMetadataKey::Schema`.
    pub const HEADER_KEY_SCHEMA: u32 = 2;
}
...
fn build_block(instant: &str) -> Vec<u8> {
    ...
    content.extend_from_slice(&wire::BLOCK_VERSION_V3.to_be_bytes());
    ...
    // Entry count, then one (key, length, bytes) triple per entry.
    header.extend_from_slice(&2u32.to_be_bytes());
    for (key, value) in [
        (wire::HEADER_KEY_INSTANT_TIME, instant),
        (wire::HEADER_KEY_SCHEMA, SCHEMA),
    ] {
    ...
    body.extend_from_slice(&wire::LOG_FORMAT_VERSION.to_be_bytes());
    body.extend_from_slice(&wire::BLOCK_TYPE_AVRO_DATA.to_be_bytes());
