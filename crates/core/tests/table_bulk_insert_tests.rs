...
async fn bulk_insert_writes_records_and_commit() -> Result<()> {
    ...
    let reopened = Table::new(base_url.path()).await?;
    let records = reopened.read_snapshot(std::iter::empty::<(&str, &str, &str)>()).await?;
    ...
    let commit_file = temp_dir
        .path()
        .join(".hoodie")
        .join(format!("{}.commit", result.instant));
...
async fn bulk_insert_rejects_partitioned_tables() -> Result<()> {
    ...
    assert!(
        error
            .to_string()
            .contains("Bulk insert currently supports only unpartitioned append-only tables")
    );
