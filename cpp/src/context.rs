...
mod tests {
    ...
    fn test_empty_context_converts() {
        ...
        assert!(ctx.reader_context.instant_range.is_none());
        assert!(ctx.reader_context.table_config.is_empty());
        assert!(ctx.reader_context.hoodie_reader_config.is_empty());
    }
    ...
    fn test_full_context_converts() {
        ...
        // ReaderContext (inner)
        assert_eq!(ctx.reader_context.table_path, "s3://bucket/table");
