...
([`cxx`](https://cxx.rs)) bindings. Apache 2.0. Rust workspace, edition `2024`. Default-feature
MSRV is `1.88`; the optional `lance` feature requires Rust `1.91+` because Lance 4.x has no
Rust-1.88-compatible Arrow 57 release. Python `>=3.10`. Key traits: async-first (tokio),
...
rustup run 1.88 cargo check --workspace --all-targets  # MSRV gate; excludes optional lance
